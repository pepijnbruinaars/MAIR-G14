...
def keyword_check_combinations(sentence: str, label: str, keypairs: list):
    ...
    for keypair in keypairs:
        ...
        # if there is some combination of the words which have all the combinations
        # words within n + 3 words of each other
        if any(
            all(keyword in split_sentence for keyword in combination)
            and abs(
                split_sentence.index(combination[0])
                - split_sentence.index(combination[-1])
            )
            <= len(combination) + 1
...
def keyword_check_patterns(sentence: str, label: str, patterns: list):
    ...
    for pattern in patterns:
        ...
        # and the 2 words are within 2 words of each other
        if (
            all(keyword in split_sentence for keyword in pattern)
            and abs(split_sentence.index(pattern[0]) - split_sentence.index(pattern[1]))
            <= 2
        ):
            return label
